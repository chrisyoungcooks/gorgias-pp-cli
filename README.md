...
**Status:** Pre-release. Read and write surfaces include dry-run request-shape coverage and confirmation guards; run tenant-specific validation in a sandbox before production mutation. See "Known gaps" below.
...
- **Writes partially dry-run covered.** All 37 write endpoints (POST/PUT/DELETE) have URL routing + HTTP method verified via `scripts/writes-shipcheck.sh` (`--dry-run` matrix, see `writes-shipcheck.json`). Live coverage breakdown:
  - **18 endpoints** — `tags`, `macros`, `teams`, `views`, `widgets`, `integrations` create/update/delete — covered by sandbox roundtrip scripts (auto-cleanup) by `scripts/writes-live-shipcheck.sh` (`writes-live-shipcheck.json`).
  - **3 endpoints** — `tickets` create/update/delete — suitable for sandbox verification in manual testing, not in recurring CI (avoids customer-facing email risk).
  - **4 endpoints** — `customers` and `custom-fields` create/update — suitable for sandbox verification by `scripts/writes-verify-once.sh` (`writes-verify-once.json`). Leaves admin-UI debris rows that need manual retiring; not in recurring CI.
...
- **No phone/voice integration coverage.** Tenants without a voice integration have no calls in these endpoints, so `phone calls-list`, `phone call-events-list`, and `phone call-recordings-list` haven't been exercised against tenant data.
...
- **Remote `search`** uses Gorgias's POST `/search` which indexes customers/agents/tags — **not tickets or messages**. For ticket/message text search, sync to local first and use `gorgias-pp-cli search <query>` against the FTS5 mirror.
